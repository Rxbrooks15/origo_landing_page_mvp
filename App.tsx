...
const GlobePage = React.lazy(() => import('./components/Globe/GlobePage'));

const App: React.FC = () => {
  const [view, setView] = useState<'home' | 'stories' | 'story-collision' | 'globe'>('home');

  if (view === 'globe') {
    return (
      <div className="bg-black min-h-screen">
        <React.Suspense fallback={null}>
          <GlobePage onBack={() => setView('home')} />
        </React.Suspense>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen flex flex-col overflow-x-hidden bg-[#0f0a1e]">
      ...
      <Navbar 
        onStoriesClick={() => setView('stories')} 
        onCollisionClick={() => setView('story-collision')}
        onGlobeClick={() => setView('globe')}
        isHome={view === 'home'} 
      />
      ...
      <main className="flex-grow pt-20 relative z-10">
        {view === 'home' ? (
          <div className="animate-in fade-in duration-700">
            ...
            {/* CreasedCosmos has been moved to Navbar discovery tab and StoriesPage */}
          ...
          <StoriesPage initialStoryId="collision" onBack={() => setView('home')} />
        )}
      </main>
      <Footer />
    </div>
